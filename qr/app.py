...
def create_pdf(ref_no, recipient, subject, intro, table_raw, closing, qr_url=None):
    ...
    # QR - 18mm, Shifted Right of Center (x=115)
    if qr_url:
    ...
    pdf.set_y(current_y + 6)
    pdf.set_x(145)
    pdf.cell(55, 7, f"Date: {datetime.now().strftime('%B %d, %Y')}", align='R')
    ...
    if subject:
        # Format: "Subject: " (Bold) + "Content" (Bold & Underlined)
        # We use write_html for the specific underline requirement
        subject_html = f"<b>Subject: <u>{subject}</u></b>"
        pdf.write_html(subject_html, line_height=6)
        pdf.ln(8)
    ...
    if table_raw.strip():
        ...
        with pdf.table(borders_layout="HORIZONTAL_LINES", line_height=6) as table:
            for d_row in rows:
                r = table.row()
...
with col2:
    subject_line = st.text_area("Subject Content (Auto-Underlined):", height=100)
...
if st.button("🚀 Generate & Host"):
    if not subject_line:
        ...
        with st.spinner("Finalizing layout..."):
            ...
            # Step 1: Pass recipient and subject to generator
